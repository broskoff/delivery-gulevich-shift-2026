...
protocol SenderContentViewProtocol: AnyObject  {
	var completionHandler: ((String, String, String, String) -> Void)? { get set }
}

final class SenderContentView: UIView, SenderContentViewProtocol {
	var completionHandler: ((String, String, String, String) -> Void)?
	var textFields: [UITextField] = []
	
	private let shipmentTopView = ShipmentTopView(title: UIConstants.Shipment.HeaderNames.senderTitle)
...
private extension SenderContentView {
	...
	func configureProgressBarComponent() {
		...
		progressBarComponent.makeComponent(currentStep: 3, totalSteps: 7)
	}
